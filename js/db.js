...
const DB_NAME = "green-ledger";
const DB_VERSION = 1;
const STORES = ["jobs", "shifts", "payslips", "files", "meta"];
const IMPORT_APPS = new Set(["green-ledger", "pay-ledger"]);
...
export async function importBackup(payload) {
  if (!payload || !IMPORT_APPS.has(payload.app)) {
    throw new Error("That file is not a GreenLedger or Pay Ledger backup.");
  }
  await clearAll();
  const db = await openDb();
  const tx = db.transaction(STORES, "readwrite");
